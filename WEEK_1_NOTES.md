...
- Q, K, V: queries, keys and values. attention(Q, K, V) is softmax
- Logits will be the scores for next character in the sequence. Embedding table (IDs idx), embedding is converting discrete tokens into dense vectors, size of embedding dimensions.